...
├── Makefile               # Сценарии, команды для сборки и запуска
├── init.py                # Начальная инициализация приложения
├── requirements.txt       # Требования для окружения Python
```

## Архитектура запуска (Docker)

Проект контейнеризован и разворачивается через `Dockerfile`.

Docker-окружение автоматически запускает три процесса:
- **Инициализация**: Подготовка баз знаний, конфигов, создание необходимых хранилищ.
- **Ожидание запроса к LLM**: Фоновый процесс слушает входящие запросы пользователя, отвечает на вопросы через языковую модель.
- **Ожидание добавления файла в базу**: Следит за входящими файлами, запускает их обработку, парсинг, добавление в базы знаний.

Это обеспечивает:
- Автоматическую подготовку среды независимо от ОС.
- Изоляцию всех зависимостей и цепочек обработки.
- Масштабируемость — каждый процесс можно запускать отдельно или в нескольких экземплярах.
